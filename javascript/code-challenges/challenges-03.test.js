...
const typeNum = (arr) => {
  return arr.filter(any => typeof any === 'number');
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(abilityData => abilityData.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  const viableData = arr.filter(abilityData => abilityData.baseStat > minBaseStat);
  return viableData.map(viable => viable.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  const nums = arr.filter(any => typeof any === 'number');
