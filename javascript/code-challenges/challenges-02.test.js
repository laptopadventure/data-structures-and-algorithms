...
const raisedToTheThird = (arr) => {
  let thirded = [];
  arr.forEach(num => thirded.push(Math.pow(num, 3)));
  return thirded;
};
...
const forEachTwoToThe = (arr) => {
  let output = [];
  arr.forEach(power => output.push(Math.pow(2, power)));
  return output;
};
...
const mapTwoToThe = (arr) => {
  return arr.map(power => Math.pow(2, power));
};
...
const extractStats = (arr) => {
  return arr.map(data => {
    let total = data.baseStat + data.effort;
    return {
      name: data.stat.name,
      total: total,
    };
